...
from utils import CIndex_lifeline, accuracy_cox, auc_formula16, cox_log_rank


def get_anchor_samples(anchor_modality, gene_common, path_common, cna_common):
    modality_map = {
        'gene': (gene_common, path_common, cna_common),
        'path': (path_common, gene_common, cna_common),
        'cna': (cna_common, gene_common, path_common),
    }
    return modality_map[anchor_modality]


def extract_tsne_features(model, full_data, device):
    # 用指定模型在完整数据集上提取 t-SNE 可视化特征
    model.eval()
    x_gene = torch.as_tensor(full_data['x_gene'], dtype=torch.float32, device=device)
    x_path = torch.as_tensor(full_data['x_path'], dtype=torch.float32, device=device)
    x_cna = torch.as_tensor(full_data['x_cna'], dtype=torch.float32, device=device)

    with torch.no_grad():
        _, _, _, _, _, _, _, x_gene_common, x_path_common, x_cna_common, x_gene_unique, x_path_unique, x_cna_unique, _ = model(
            x_gene, x_path, x_cna
        )

    return {
        'original': {
            'gene': x_gene.detach().cpu().numpy(),
            'path': x_path.detach().cpu().numpy(),
            'cna': x_cna.detach().cpu().numpy(),
        },
        'specific': {
            'gene': x_gene_unique.detach().cpu().numpy(),
            'path': x_path_unique.detach().cpu().numpy(),
            'cna': x_cna_unique.detach().cpu().numpy(),
        },
        'aligned': {
            'gene': x_gene_common.detach().cpu().numpy(),
            'path': x_path_common.detach().cpu().numpy(),
            'cna': x_cna_common.detach().cpu().numpy(),
        },
    }
...
def train(opt, data, device, k):
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter + opt.niter_decay + 1)):
        ...
        for x_gene, x_path, x_cna, censor, survtime in train_loader:
            ...
            pred, x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, _ = model(
                x_gene.to(device), x_path.to(device), x_cna.to(device)
            )

            optimizer_d.zero_grad()
            # 共享表征中由锚点模态充当 reference，其余模态向其对齐
            real_samples, fake_samples_1, fake_samples_2 = get_anchor_samples(opt.anchor_modality, x7, x8, x9)
            real_samples = real_samples.detach().requires_grad_(True)
            fake_samples_1 = fake_samples_1.detach().requires_grad_(True)
            fake_samples_2 = fake_samples_2.detach().requires_grad_(True)
            ...
            total_loss = (
                loss_cox
                 ...
                 + opt.lambda_recon * rec_loss
                 + opt.lambda_orth * diff_loss
                 + opt.lambda_r3gan * g_loss_adv  # LUSC 消融实验：暂时去掉 R3GAN 对应的对抗对齐损失
            )
        ...
        if opt.measure or epoch == (opt.niter + opt.niter_decay - 1):
            ...
            _, cindex_test, pvalue_test, surv_acc_test, auc_test, pred_test = test(opt, model, data, 'test', device)
            ...
            if not getattr(opt, 'skip_train_artifacts', 0):
                pickle.dump(
                    pred_test,
                    open(
                        os.path.join(
                            ...
                            opt.model_name,
                            f'{k}_fold',
                            f'{opt.model_name}_{epoch}_pred_test.pkl',
                        ),
                        'wb',
                    ),
                )
        ...
        _, val_cindex, _, _, _, _ = test(opt, model, data, 'val', device)
        ...
        if val_cindex > best_val_cindex + min_delta:
            ...
            if not getattr(opt, 'skip_train_artifacts', 0):
                torch.save(
                    model.state_dict(),
                    os.path.join(opt.model_save, opt.exp_name, opt.model_name, f'best_model_fold_{k}.pt'),
                )
        else:
    ...
    tsne_features = extract_tsne_features(model, data['test'], device)
    return model, optimizer, metric_logger, tsne_features
...
def test(opt, model, data, split, device):
    ...
    with torch.no_grad():
        for x_gene, x_path, x_cna, censor, survtime in test_loader:
            censor = censor.to(device)
