...
def set_random_seed(seed):
    # 统一固定 Python / NumPy / PyTorch 随机性，便于复现实验
    random.seed(seed)
    np.random.seed(seed)
...
def train(opt,data,device,k):
    ...
    train_loader = DataLoader(dataset = custom_data_loader, batch_size = len(custom_data_loader), num_workers =0, shuffle = False)
    ...
    for epoch in tqdm(range(opt.epoch_count, opt.niter+opt.niter_decay+1)):
        ...
        _, val_cindex, _, _, _ = test(opt, model, data,'val', device)
...
def test(opt,model, data, split, device):
    ...
    test_loader = DataLoader(dataset = custom_data_loader,  batch_size = len(custom_data_loader),num_workers = 0, shuffle = False)
    risk_pred_all, censor_all, survtime_all = np.array([]), np.array([]), np.array([])
    loss_test = 0
