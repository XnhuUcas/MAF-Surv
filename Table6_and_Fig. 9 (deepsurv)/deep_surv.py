...
# 兼容旧版 Theano/Lasagne 对 np.bool 的依赖
...
# 兼容新版 Theano 中移除的 theano.tensor.signal.downsample
try:
...
try:
    ...
    try:
        from lasagne.nonlinearities import ScaledExponentialLinearUnit as selu
    except ImportError:
        def selu(x):
            return rectify(x)
...
class DeepSurv:
    ...
    def _negative_log_likelihood(self, E, deterministic = False):
        ...
        """
        # 统一压成一维，避免旧版 Theano 在 (n, 1) / (n,) 广播上报错
        risk = self.risk(deterministic).flatten()
    ...
    **kwargs):
        ...
        # 旧版 Theano 在 0.0 * 某些 broadcast 张量上容易触发图优化错误，
        # 因此正则系数为 0 时直接不把对应项拼进计算图。
        if L1_reg:
            loss = loss + regularize_layer_params(self.network, l1) * L1_reg
        if L2_reg:
            loss = loss + regularize_layer_params(self.network, l2) * L2_reg
    ...
    **kwargs):
    ...
    def get_concordance_index(self, x, t, e, **kwargs):
        ...
        partial_hazards = numpy.asarray(compute_hazards(x)).reshape(-1)
    ...
    **kwargs):
    ...
    def predict_risk(self, x):
        ...
        return numpy.asarray(risk_fxn(x)).reshape(-1)
